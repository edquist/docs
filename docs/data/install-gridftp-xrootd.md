...
<span class="twiki-macro SPACEOUT">Install !Xrootd !GridFTP</span>
==================================================================
...
For a full storage element, visit Documentation/Release3.InstallBestmanXrootdSE. However, bigger sites (tier-2, etc) may require multiple GridFTP to be load balanced under a single BeStMan Gateway SRM interface. This page aims to explain how to install such a GridFTP server.
...
The GridFTP server provides high-performance, secure and reliable data transfer. This guide is primarily intended for installations that require one [BeStMan](InstallBestmanXrootdSE) endpoint but multiple GridFTP servers for scalability. Multiple GridFTP instances on different servers are recommended if:
...
Note that this package is primarily intended for GridFTP acting as an interface for XRootD server, usually part of a bigger storage element installation. If you have not installed an XRootD server yet, follow the instructions in Documentation/Release3.InstallXrootd.
...
<span class="twiki-macro INCLUDE" section="dsi">InstallBestmanXrootdSE</span>
...
``` file
supportedProtocolList=gsiftp://%RED%gridftp.server.tld%ENDCOLOR%;gsiftp://%RED%gridftp2.server.tld%ENDCOLOR%;gsiftp://%RED%gridftp3.server.tld%ENDCOLOR%
```

Configuring xrootdfs
--------------------

<span class="twiki-macro INCLUDE" section="mount">InstallBestmanXrootdSE</span>

(Optional) Configuring secured xrootdfs
---------------------------------------

<span class="twiki-macro INCLUDE" section="secured">InstallBestmanXrootdSE</span>

Configure Xrootd Gratia Probes
------------------------------

<span class="twiki-macro INCLUDE" section="gratia">InstallBestmanXrootdSE</span>

Note that you can also enable the GridFTP gratia probe. However, the XRootD probes are likely sufficient. More information on the GridFTP probe can be found here: [InstallOSGGridFTP\#6\_0\_Gratia\_GridFTP\_Transfer\_Prob](InstallOSGGridFTP#6_0_Gratia_GridFTP_Transfer_Prob)
...
<span class="twiki-macro INCLUDE" section="gratia2start">InstallBestmanXrootdSE</span>
...
\* (other grid service running on the machine may still use it) <span class="twiki-macro INCLUDE" section="OSGBriefFetchCrlStop">InstallCertAuth</span>
...
<span class="twiki-macro INCLUDE" section="gratia2stop">InstallBestmanXrootdSE</span>
